...
for link in links: 
    ...
    POSTS = SOUP.find_all(class_='col-12 col-lg-10')
    for post in POSTS:
        #print(post)
...
for link in links:
    ...
    if money_goal is not None:
        newDict['Money-Goal'] = money_goal.get_text().strip()
        #print(newDict['Money-Goal'])
    goal = SOUP.find(class_='goal')
    if goal is not None:
        newDict['Goal'] = goal.get_text().strip()
    ...
    if ratingResult is not None:
        ...
        for link in social_links:
            if len(link.find('i')) > 0 is not None:
